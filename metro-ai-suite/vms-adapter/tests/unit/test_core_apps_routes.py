...
"""Tests for the Core App registry + dynamic discovery routes (LVC only)."""
...
from core_app_shim.lvc.config import LiveCaptioningCoreAppConfig
...
def fastapi_client(app_config, monkeypatch):
    registry = ShimFactory.create_core_app_shims(app_config)

    async def _available_true(self):  # noqa: ANN001
        return True
    monkeypatch.setattr(LiveCaptioningCoreAppShim, "is_available", _available_true)
    ...
    monkeypatch.setattr(LiveCaptioningCoreAppShim, "start", _start)

    set_shims([], registry, app_config)

    app = FastAPI()
    app.include_router(core_apps_routes.router, prefix="/v1")
...
def test_legacy_core_app_singular_converts_to_list():
    ...
    assert len(cfg.core_apps) == 1
    assert cfg.core_apps[0].type == "live_captioning"
    assert cfg.core_app is cfg.core_apps[0]


def test_discover_returns_schemas_and_availability(fastapi_client):
    client, _ = fastapi_client
    resp = client.get("/v1/core-apps/discover")
...
def test_schema_endpoint(fastapi_client):
    client, _ = fastapi_client
    resp = client.get("/v1/core-apps/live_captioning/schema")
    assert resp.status_code == 200
    schema = resp.json()
...
def test_schema_unknown_app_returns_404(fastapi_client):
    client, _ = fastapi_client
    resp = client.get("/v1/core-apps/nope/schema")
    assert resp.status_code == 404


def test_start_validates_payload(fastapi_client):
    client, _ = fastapi_client
    resp = client.post("/v1/core-apps/live_captioning/start", json={})
...
def test_start_dispatches_to_shim(fastapi_client):
    ...
    resp = client.post(
        "/v1/core-apps/live_captioning/start",
...
def test_start_unknown_app_returns_404(fastapi_client):
    client, _ = fastapi_client
    resp = client.post("/v1/core-apps/nope/start", json={"cameraId": "x"})
    assert resp.status_code == 404


def test_lvc_shim_metadata():
    assert LiveCaptioningCoreAppShim.app_id == "live_captioning"
    schema = LiveCaptioningCoreAppShim.param_model.model_json_schema()
    assert schema["type"] == "object"
    assert "cameraId" in schema["properties"]
