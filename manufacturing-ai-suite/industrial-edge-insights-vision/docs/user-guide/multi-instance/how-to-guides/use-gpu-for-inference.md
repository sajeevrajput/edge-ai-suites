...
The pipeline `pallet_defect_detection_gpu` for pallet defect detection contains GPU specific elements and uses GPU backend for inferencing. We can start the pipeline as follows:

```sh
./sample_start.sh -i <INSTANCE_NAME> -p pallet_defect_detection_gpu
```